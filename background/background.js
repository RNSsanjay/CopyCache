...
// Store for tracking side panels
...
async function openFallbackTab() {
  try {
    const tabs = await chrome.tabs.query({ url: chrome.runtime.getURL('popup/build/index.html') });
    ...
    if (tabs.length > 0) {
      ...
      await chrome.tabs.create({
        url: chrome.runtime.getURL('popup/build/index.html'),
        active: true
      });
    }
  } catch (error) {
...
function toggleCopyCacheSidePanel() {
  ...
  window.openFullCopyCache = () => {
    window.open(chrome.runtime.getURL('popup/build/index.html'), '_blank');
  };
